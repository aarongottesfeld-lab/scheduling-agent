...
//
// A persistent Express process that exposes Rendezvous scheduling tools
// to AI clients (Claude Desktop, GPT, etc.) via the Model Context Protocol.
//
// Transport: SSE (Server-Sent Events)
//   GET  /sse     — establishes SSE stream (requires auth)
//   POST /messages — receives MCP messages (requires auth)
//
// Auth: Bearer token via Authorization header
//   - MCP_API_KEY (dev shortcut) → maps to MCP_OWNER_USER_ID
//   - OAuth tokens stored in mcp_tokens table (24h expiry)
//
// Architecture: one McpServer instance per SSE connection. Tool callbacks
// close over the authenticated userId so every DB query is scoped to the
// correct user. The session map tracks transport ↔ userId ↔ McpServer.
//
...
// ---------------------------------------------------------------------------
const rateLimitWindow = new Map(); // userId → [timestamp, ...]
const RATE_LIMIT = 60;
...
// ---------------------------------------------------------------------------
// Maps transport sessionId → { transport, userId, mcpServer }
const sessions = {};
